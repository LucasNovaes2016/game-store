...
const GameItem = ({ game, deleteGame, setCurrentGame }) => {
  ...
  const [edit_game_mode, setEditGameMode] = useState(false);

  const setEditGame = (game) => {
    setCurrentGame(game);
    setEditGameMode(true);

  }

  return (
    <Fragment>
      {edit_game_mode ? <Redirect to="/games/edit" /> : null}
          ...
          <div className="card-footer game-card-footer">
            <div className="d-flex justify-content-between">
              <div data-toggle="tooltip" data-placement="left" title="Editar Jogo">
                <a href='#!' onClick={() => setEditGame(game)} className="fa-stack fa-lg text-primary">
